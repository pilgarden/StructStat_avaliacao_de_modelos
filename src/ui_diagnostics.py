...
def render_diagnostics():
    if "df_global" not in st.session_state or st.session_state["df_global"] is None:
        st.warning("Carregue um arquivo de dados na barra lateral.")
        return
    ...
    if not preditores or not alvo or not previsto or alvo == previsto:
        st.info("Configure as variáveis na barra lateral (Real, Prevista e Independentes).")
        return
    ...
    with tab2:
        ...
        try:
            ...
            if std_resid == 0 or len(residuos) == 0:
                st.warning("Não há variabilidade nos resíduos para identificar outliers.")
            else:
                z_scores = np.abs((residuos - residuos.mean()) / std_resid)
                ...
                with c_left:
                    fig_out = px.scatter(
                        x=df_clean[previsto], y=residuos, color=outliers_z,
                        title="Identificação de Outliers ($|Z| > 3$)"
                    )
                ...
                with c_right:
                    ...
                    if pct > 5:
                        ...
                        st.success("✅ Percentagem controlada.")
                    if is_outlier:
                        st.warning(f"Teste de Grubbs detetou outlier extremo no índice {idx}.")
                    st.download_button(
                        "⬇️ Baixar Outliers",
                        df_clean[outliers_z].to_csv().encode('utf-8'),
                        "outliers.csv",
                        "text/csv",
                    )
        except Exception as e:
            ...
            if st.button("🚀 Executar Simulação de Monte Carlo (Sobol)", type="primary"):
                with st.spinner("A treinar Superfície de Resposta e a amostrar Matrizes de Saltelli..."):
                    ...
                    if Si is not None:
                        ...
                        st.session_state['sobol_results'] = df_si
